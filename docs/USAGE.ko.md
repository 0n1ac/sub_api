...
Backend tool을 사용한 경우 `sub_api.tools`와 CLI의 `tools:` stats에 감지된 tool 이름이 표시됩니다. Gemini는 `SUB_API_GEMINI_DISABLE_TOOLS=true` 또는 CLI의 `--disable-tools` 옵션으로 tool-heavy 동작을 끌 수 있습니다. 현재 Gemini CLI는 tool을 0개로 설정하면 API 오류가 발생하므로, `sub_api`는 검색, fetch, 파일, shell, MCP, extension, skill tool이 노출되지 않도록 최소 built-in allowlist로 제한합니다.
